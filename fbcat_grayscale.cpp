...
static inline unsigned char get_grayscale(unsigned int pixel,
  ...
  unsigned char r =
      ...
      8;
  unsigned char g = colormap->green[(pixel >> info->green.offset) &
                                    ((1 << info->green.length) - 1)] >>
                    8;
  unsigned char b = colormap->blue[(pixel >> info->blue.offset) &
                                   ((1 << info->blue.length) - 1)] >>
                    8;
  return (unsigned char)(0.3 * r + 0.59 * g + 0.11 * b);
}

static void dump_video_memory_grayscale(const unsigned char *video_memory,
                                        const struct fb_var_screeninfo *info,
                                        const struct fb_cmap *colormap,
...
int main(int argc, const char **argv) {
  ...
  dump_video_memory_grayscale(video_memory, &var_info, &colormap,
